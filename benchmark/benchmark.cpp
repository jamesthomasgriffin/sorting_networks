...
public:
	inline IntSwap(int& x, int& y) {
		int dx = x, dy = y, tmp; tmp = x = dx < dy ? dx : dy; y ^= dx ^ tmp;
	}
};
...
template<class F>
auto benchmark(F&& f, int n_tests, int n_elements)
{
...
void run_benchmarks(int num_tests, OS& ostr) {
	...
	// Vanilla std::sort
	results[result_ix++] = benchmark([](auto& data, auto n_tests, auto n_elements) {
	...
	// sorting network from Vectorized/static-sort, uses similar methods to our own
	results[result_ix++] = benchmark([](auto& data, auto n_tests, auto n_elements) {
	...
	results[result_ix++] = benchmark([](auto& data, auto n_tests, auto n_elements) {
		...
		using network_type = typename sorting_networks::SortingNetwork<NumElements>::template Sort<int*, IntSwap, 0, 1, NumElements>;
		int* ptr = data.data();
		for (int i = 0; i < n_tests * n_elements; i += NumElements)
		{
			int* ptr2 = ptr + i;
	...
	// the same network again, however this sorts 8 sequences at once using AVX2 instructions
	results[result_ix++] = benchmark([](auto& data, auto n_tests, auto n_elements) {
...
int main(int argc, const char* argv[])
{
	...
	std::string names[n_benchmarks] = { "std::sort", "qsort", "static_sort", "jtg_sort", "jtg_sort_intswap", "jtg_sort_avx2" };
	ostr << "NumElements";
	...
	constexpr_for<Start, End, 1>([&ostr](auto N) {
		run_benchmarks<N>((1000000 / N) * 8, ostr);
		});
