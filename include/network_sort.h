...
#include <array>  // std::array used for reordering data ready for simd sorting
...
namespace sorting_networks
{
	...
	public:
		...
		template<typename C>
		inline void operator()(C& container) {
			Sort<C, swappers::DefaultSwap<typename C::value_type>, 0, 1, N> sort(container);
		}

		template<typename C>
		inline void operator()(C* data) {
		...
		public:
			inline void operator()(int* container) {
				...
				for (int i = 0; i < N; ++i)
					block[i] = gather8i<0, N, 2 * N, 3 * N, 4 * N, 5 * N, 6 * N, 7 * N>(&container[i]);
				Sort<decltype(block), swappers::MinMaxSwap<Vec8i>, 0, 1, N> s(block);
				for (int i = 0; i < N; ++i)
					scatter<0, N, 2 * N, 3 * N, 4 * N, 5 * N, 6 * N, 7 * N>(block[i], &container[i]);
			}
		};
#endif
		...
		struct Sort {
			inline Sort(C& container) {
				static_assert(A >= 3, "This code path should not be sorting too small blocks");
	...
	private:
		...
		struct Merge {
			inline Merge(C& container) {
				...
				Merge<C, Swap, Off, 2 * Str, A / 2, (B + 1) / 2> merge_odd_values(container);
				Merge<C, Swap, Off + Str, 2 * Str, A / 2, B / 2> merge_even_values(container); 

				Finalise<C, Swap, Off + Str, Str, (A + B - 1) / 2> f(container);
				//for (int i = 0; i < (A + B - 1) / 2; ++i)  // Force unrolling of this?
				//	Swap s(container[Off + Str * (2 * i + 1)], container[Off + Str * (2 * i + 2)]);
			}
		};
	...
	{
		// A dummy container that when subscripted returns a struct containing that subscript
		// and an output stream
		template<class OS>
		class OutputContainerPassthrough {
		public:
			struct Lane {
				unsigned int place;
			...
			OutputContainerPassthrough(OS& os, unsigned int num_lanes) : m_os{ os }, m_num_lanes{ num_lanes }{}
		...
		// Does not perform swapping, but instead outputs characters representing the swap to
		// an output stream
		template<class OS>
		class OutputSwap {
		public:
